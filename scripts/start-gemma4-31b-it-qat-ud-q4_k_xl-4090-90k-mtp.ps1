...
$qatModelRoot = Join-Path $workspace 'models\unsloth-gemma4-qat'
$target = Join-Path $qatModelRoot 'gemma-4-31B-it-qat-UD-Q4_K_XL.gguf'
$mtpHead = Join-Path $qatModelRoot 'MTP\mtp-gemma-4-31B-it-Q8_0.gguf'
...
if (-not (Test-Path -LiteralPath $target -PathType Leaf)) {
    throw "The Gemma 4 31B Instruct QAT target is missing: $target"
}
if (-not $NoMtp -and -not (Test-Path -LiteralPath $mtpHead -PathType Leaf)) {
    throw "The matching Gemma 4 31B Instruct QAT MTP drafter is missing: $mtpHead"
}
...
$alias = "gemma4-31b-it-qat-ud-q4-k-xl-90k-$mode"
